...
public:
	...
	constexpr GrayD(const C& rgb)
		: ProportionalUnsigned<D>((
			2125 * (modm::WideType<T>)(GrayD(rgb.red())) +
			7154 * (modm::WideType<T>)(GrayD(rgb.green())) +
			0721 * (modm::WideType<T>)(GrayD(rgb.blue()))
			) / 10000
		)
	{}
	...
	// It feels natural to use hsv.value() like it is for now
 	template<ColorHsv C>
