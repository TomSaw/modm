...
/** @brief Enable compile time calculations by defining constexpr friend operators
 *
...
constexpr C operator+(C const& lrgb, C const& rrgb) {
	return {
		lrgb.getRed() + rrgb.getRed(),
		lrgb.getGreen() + rrgb.getGreen(),
		lrgb.getBlue() + rrgb.getBlue()
	};
}
