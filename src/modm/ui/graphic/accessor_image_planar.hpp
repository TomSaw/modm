...
class ImageAccessor<C, Accessor> : public Accessor<const C>
{
public:
	ImageAccessor() = default;

	ImageAccessor(const BufferInterface<C>* buffer, shape::Point placement = {0, 0})
		: Accessor<const C>(reinterpret_cast<const C*>(buffer->virtualBuffer())), section(placement, placement + buffer->virtualSize())
	{
	...
	{
		incrementCol();
		return Accessor<const C>::operator*();
	}
