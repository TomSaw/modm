...
Ssd1306<I2cMaster, H>::clear(ColorType color)
{
	...
	// FIXME color::Gray8(color).value is not very accurate
	// Fill commandBuffer with 0x00 or 0xFF
	std::fill(std::begin(commandBuffer), std::end(commandBuffer), color::Gray8(color).value());
	transaction_success = true;

	this->clipping = shape::Section({0, 0}, {128, H});
	RF_CALL(updateClipping());
